...
public interface SaleFigureEmployeeRepository extends JpaRepository<Invoice, Long> {
    ...
    @Query(value = "SELECT i.id_employee AS id, u.full_name AS fullName ,sum(total_money) AS total, MONTH(i.updated_at) AS month FROM invoice i JOIN user u ON i.id_employee = u.id WHERE u.id_role = 2 GROUP BY i.id_employee", nativeQuery = true)
