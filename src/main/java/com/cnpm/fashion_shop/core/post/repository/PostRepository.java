...
public interface PostRepository extends JpaRepository<Post, Long> {
    ...
    @Query(value = "SELECT post.content, post.id, image.link as Link, image.name as Name FROM (post inner join image on post.id_image = image.id) WHERE post.is_deleted = false ", nativeQuery = true)
