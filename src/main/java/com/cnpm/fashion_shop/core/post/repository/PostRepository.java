...
public interface PostRepository extends JpaRepository<Post, Long> {
    ...
    @Query(value = "SELECT post.content, post.id, image.link as image_link, image.name as name_image FROM (post inner join image on post.id_image = image.id) WHERE post.is_deleted = false ", nativeQuery = true)
